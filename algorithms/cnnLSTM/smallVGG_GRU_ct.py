...
if __name__ == '__main__':
    ...
    for train, test in kfold.split(model_images):
        ...
        '''base_input = layers.Input(batch_shape=(args.batch_size, args.width, args.height, args.channels))

        base_model = layers.Conv2D(32, (3, 3), padding="same", activation="relu")(base_input)
        base_model = layers.BatchNormalization(axis=2)(base_model)
        base_model = layers.MaxPool2D(pool_size=(3, 3))(base_model)
        base_model = layers.Dropout(0.5)(base_model)
        # (CONV => RELU) * 2 => POOL
        base_model = layers.Conv2D(64, (3, 3), padding="same", activation = "relu")(base_model)
        base_model = layers.BatchNormalization(axis=2)(base_model)
        base_model = layers.Conv2D(64, (3, 3), padding="same", activation="relu")(base_model)
        base_model = layers.BatchNormalization(axis=2)(base_model)
        base_model = layers.MaxPool2D(pool_size=(2,2))(base_model)
        base_model = layers.Dropout(0.25)(base_model)
        # (CONV => RELU) * 2 => POOL
        base_model = layers.Conv2D(128, (3, 3), padding="same", activation="relu")(base_model)
        base_model = layers.BatchNormalization(axis=2)(base_model)
        base_model = layers.Conv2D(128, (3, 3), padding="same", activation="relu")(base_model)
        base_model = layers.BatchNormalization(axis=2)(base_model)
        base_model = layers.MaxPool2D(pool_size=(2,2))(base_model)
        ...
        base_model = layers.Dense(1024, activation="relu")(base_model)
        ...
        model.add(GRU(3, activation='tanh', recurrent_activation='sigmoid'))
